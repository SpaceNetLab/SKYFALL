...
if __name__ == "__main__":
    ...
    user_connect_sat = np.loadtxt('../' + cons_name + '/circle_data/link_traffic_data/0/user_connect_sat.txt')
    print("Starting aggregating for circle...")
    for time_slot in range(total_time):
        bot_block_bot_num_per_block = np.loadtxt('../' + cons_name + '/circle_data/attack_traffic_data_land_only_bot/' + str(ratio) + "-" +
        ...
        if bot_block_bot_num_per_block.ndim == 0:
            bot_block_bot_num_per_block = np.array([bot_block_bot_num_per_block])
        bot_block = np.loadtxt('../' + cons_name + '/circle_data/attack_traffic_data_land_only_bot/' + str(ratio) + "-" +
            str(traffic_thre) + "-" + str(sat_per_cycle) + "-" +
            str(GSL_capacity) + "-" + str(unit_traffic) + '/' + str(time_slot) + '/bot_block.txt')
